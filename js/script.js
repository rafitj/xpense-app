...
const makeSearch = () => {
  ...
  if (query !== "") {
    const newInstance = Singleton.getInstance().filter(
      page =>
        page.created.match(query) ||
        page.merchant.toLowerCase().includes(query) ||
        (page.amount / 100).toString().includes(query)
    );
    const filteredNewInstance = filterTable(newInstance);
...
const sortTable = () => {
  ...
  if (sortBy === "Merchant") {
    if (ascending)
      pageInstance.sort((a, b) => a.merchant.localeCompare(b.merchant));
    else pageInstance.sort((a, b) => b.merchant.localeCompare(a.merchant));
  } else if (sortBy === "Date") {
    if (ascending)
      pageInstance.sort((a, b) => (a.created < b.created ? 1 : -1));
...
const addToTable = (transaction, isNew) => {
  ...
  let amountClass;
  if (amount !== 0) {
    amountText = `${amount > 0 ? "+ " : "- "}$${Math.abs(amount / 100).toFixed(
      2
    )}`;
...
const renderTable = page => {
  ...
  if (pageInstance.length === 0 && !$("#no-search-results").length) {
    ...
    if (parseInt(page) + 2 <= numPages) {
      $("#next-next-page")
        .html(`${parseInt(page) + 2}`)
        .data("page", parseInt(page) + 2)
        .show();
    }
